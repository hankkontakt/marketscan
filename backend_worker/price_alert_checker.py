...
Usage:
    python backend_worker/price_alert_checker.py

Requires DATABASE_URL in .env or environment (Session Pooler, port 6543).
"""
import os
import sys
import logging
...
def check_and_trigger(
    ...
    """Check if alert condition is met and mark as triggered."""
    triggered = False
...
def run_check(dsn: str | None = None) -> tuple[int, int]:
    ...
    with psycopg2.connect(dsn) as conn, conn.cursor() as cur:  # type: ignore
        ...
        missing = [
            t for t in unique_tickers if t not in prices or prices[t] is None
        ]
        if missing:
            logger.info(
                "Tickers not found in scan_results, trying yfinance: %s", missing
            )
        ...
        for alert in alerts:
            ...
            if current_price is None:
                logger.warning(
                    "No price available for %s (alert %s) -- skipping", ticker, alert_id
                )
                continue
            ...
            if check_and_trigger(cur, alert_id, current_price, target, condition):
                triggered += 1
                logger.info(
