...
|---|---|---|---|
| 2026-06-09 | feat | **Company profiles — yfinance bolagsbeskrivning per aktie.** Ny `company_profiles`-tabell (migration 026): `description` (longBusinessSummary, English), `employees`, `website`, `industry`, `country`, `beta`, `week_52_high`, `week_52_low`. Ny `backend_worker/company_info_fetcher.py`: hämtar via `yf.Ticker.info`, upsert med psycopg2, stöder enstaka ticker eller alla i scan_results (~0.4s/ticker, ~8 min för 1200 aktier). Körs automatiskt efter weekly/smallcap i `entrypoint.py`. Ny GH Actions `company_profiles.yml` (söndagar 07:00 + workflow_dispatch med valfritt ticker-input). API: ny `GET /api/stocks/{ticker}/profile` i `stocks.py` (returnerar 404 om ej hämtad ännu). Frontend: `useCompanyProfile` hook + `CompanyProfile` interface i `useStock.ts`. UI: ny `CompanyProfileCard` i `OverviewTab` i `StockView.tsx` — visar beskrivningstext (collapsible vid >320 tecken), industry/country/employees/website-rad, 52-veckors prisintervallfält med gradient-bar + rörlig prick för nuvarande kurs, beta + uppdateringsdatum. Sparad idé: `--translate`-flagga + `description_sv`-kolumn för framtida AI-översättning via DeepSeek. | `026_company_profiles.sql`, `company_info_fetcher.py`, `company_profiles.yml`, `stocks.py`, `useStock.ts`, `StockView.tsx` |
