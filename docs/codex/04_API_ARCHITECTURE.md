...
7. **Frontend-kontrakt:** `scripts/generate_v3_types.py` genererar `apps/web/lib/types/decision_v3.ts` från OpenAPI — redigera aldrig för hand; `python scripts/generate_v3_types.py --check` (kört i `apps/api/tests/test_v3_types_sync.py`) är drift-gaten. Frontend-gaten är `NEXT_PUBLIC_DECISIONS_V3=true` (se `apps/web/lib/v3.ts`), oberoende av API-flaggan.
