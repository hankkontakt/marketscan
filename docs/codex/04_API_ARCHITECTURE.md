...
6. **V3 endpoints:** `GET /api/v3/decisions/screener` (filter: thesis_band/setup_state/risk_state/data_grade/segment), `GET /api/v3/decisions/stock/{listing_id}` (accepterar även legacy-ticker-alias, ej UUID → ticker-slagning i publicerad vy), `.../history`, `.../evidence`, `.../changes`, `GET /api/v3/decisions/system/current-snapshot` (publiceringspekare + manifest/actionable/excluded-räknare). Alla svar är projektioner av `current_decisions_v3`-vyn (083+085) med name/segment/price/change_pct från legacy-rad under migreringsfönstret.
7. **V3 Phase 9 endpoints (2026-09-01):** `GET /api/v3/decisions/changes?limit=` läser `decision_transitions` (worker-skriven diff-tabell, 0 rader → 200 tom lista, aldrig 404; snapshot-meta från publication_state), `GET /api/v3/decisions/transitions?limit=` (samma data normaliserad för larm), `POST /api/v3/decisions/compare` (body `{tickers: [..]}`, samma snapshot för alla träffar; 0 träffar → 404, blandade snapshots → 409). Klientfunktioner `v3Changes/v3Transitions/v3Compare` i `apps/web/lib/v3.ts`.
8. **V3-diff-lager:** API:t diffar ALDRIG snapshots själv (RLS visar anon bara `status='PUBLISHED'`, 083:281, och publish supersedear gamla, 083:237-238). `backend_worker/decision_transitions.py` körs efter publikation (service-roll) och skriver `decision_transitions`-rader (thesis/setup/risk/data_grade/tradability/rank, rank |Δ|≥5, reason codes deterministiska, aldrig LLM). API:t läser bara tabellen — "workers compute, API reads" (spec §31).
9. **Portfolio V3-berikning:** `enrich_with_v3_decisions` (apps/api/core/enrichment.py) joinar `current_decisions_v3` via ticker och lägger additivt thesis_band/setup_state/risk_state/data_grade/decision_id/master_rank_score/segment_percentile/tradability_state/is_actionable/v3_snapshot_id i `GET /api/portfolio`-holdings och risk-_get_holdings_with_prices. `construct` omställs inte (spec §0.3).
