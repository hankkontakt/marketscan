...
|---|---|---|---|
| 2026-08-29 | mega-natt | **Nattrundan: full felsöknings-+förbättringsvåg.** P0-buggfixar: (1) Route-kollision `/api/alerts` (alerts.py skuggade smart_alerts) → `/api/price-alerts`; (2) QMJ rankade bara 120 tickers (alfabetisk, ingen OFFSET) → limit 0 = alla; (3) PIT-gate (fy_end+5mån) gater nu ranken + **fallback till senast giltiga årsbokslut** per bolag; (4) intcov inverterad (yfinance negativ Interest Expense → abs); (5) mass-falsk-delisting vid Yahoo-avbrott (probe True/False/None); (6) surge-mätning var invers frekvens → äkta 48h-ratio; (7) psycopg2 `INTERVAL '%s'`-dubbelquote-syntaxfel (dedup + count_hits); (8) klusterscore-inflation via mcap=1 → None; (9) insider-dubbelräkning FI↔Finnhub → dedupe_trades; (10) confidence 0.0-bevaring + bearing-normalisering. Nytt: QMJ-faktorregim (AQR xlsx → OOS-percentil, migration 048), TS-SUE + PIT-snapshot (migration 051), sektorrelativt värde (migration 050), IC-ärlighet + cluster-union + dilution + data_quality i radarn, holdings-fallback + NCAB/TagMaster-seed + norm_ticker_input, LEI/isin-cache → worker_state, FI CSV-export-datumfix, /scan default alla segment, CORS_ORIGINS-prod-fix, SHA-pinning + dependabot, migrationer 048-052. Tester: 200 backend + 41 API. | See commits 4d1c549…0a8b358 |
| 2026-06-10 | mega | **Spec 11 Scoring v2 (S5, S1, S2, S3) + Spec 12 LLM v2 (L1, L2, L3, L4).** Scoring v2: **S5** — 4 extra features (insider_cluster, cluster_score, qualitative_score, mews_score) i RANKER_FEATURES + BACKTEST_SAFE_FEATURES-lista i `core/ml_ranker.py`. **S1** — `label_uniqueness()` + `combine_weights()` i `core/ml_validation.py`; integrated i `_fit_model` via `use_uniqueness=True`. **S2** — `core/meta_labeling.py` (NY) med `triple_barrier_labels()`, `train_meta_model()`, `apply_meta()`. **S3** — `core/conformal.py` (NY) med split-conformal `calibrate()` + `predict_interval()`. `scripts/eval_model.py`: nytt `permutation-importance`-kommando (S5 gate). 3 nya test-filer (uniqueness, meta_labeling, conformal). LLM v2: **L1** — `apps/api/core/reranker.py` (NY) med lazy-loaded CrossEncoder + fallback; integrated i `extract_signals.py`. **L2** — `apps/api/core/grounding.py` (NY) med `require_citations()`. **L3** — strict schema validation + retry i `llm_complete()`. **L4** — self-consistency (2× synthesis) i `get_committee_analysis`. `sentence-transformers` pinnad i backend_worker requirements. 33/33 tester pass. | Se spec 11, 12 + nya filer |
| 2026-06-10 | mega | **Batch 1: ML-ranker (#1) + MEWS (#3) + FI Insider (#5) + Dokintelligens (#7+#12) + Regim (#15) + Black-Litterman (#19).** Alla 6 projekt i master-planen implementerade. **#1**: `core/ml_validation.py` (purged walk-forward + DSR), `_walk_forward_validate`/`evaluate_model` migrerade till purged folds, `_fit_model` med `objective_mode` (lambdarank_ndcg/rank_ic/xgboost_cs), `scripts/eval_model.py` (compare-objectives, gate), `scripts/audit_baseline.py`. **#3**: `smallcap/mews.py` (6-faktor MEWS 0-100 + flagga), `tests/test_mews.py`, pipeline-integration, migration 028. **#5**: `fi_insider_bulk.py` (bulk-ingestion FI-register med paginering + 0-rader-larm), `insider_cluster.py` (kluster-scoring), migration 029, workflow `fi_insider.yml`. **#7+#12**: `apps/api/core/llm_client.py` (LLM-abstraktion med Gemini→DeepSeek-fallback + cache + budget), `rag/document_fetcher.py` + `rag/extract_signals.py`, migration 030, workflow `doc_intelligence.yml`. **#15**: `core/regime_hmm.py` (3-state HMM), `core/regime_ensemble.py` (regimviktad ensemble med uncertainty), migration 031. **#19**: `apps/api/core/portfolio_construction.py` (ERC + Black-Litterman), `prices.fetch_price_history_batch()`, `POST /api/portfolio/construct` i portfolio.py, migration 032. Nya workflows registrerade i admin.py + AdminSections.tsx. db_loader SCAN_COLUMNS utökad. outcome_filler dynamisk model_version. Requirements: hmmlearn, beautifulsoup4, lxml, pdfplumber/pypdf. 25 nya tester pass. | Se respektive spec |
| 2026-06-09 | fix | **Admin-pipeline: fix GitHub 422 + exponera alla 13 workflows.** (1) `pipeline.yml`: ändrade `type: choice` → `type: string` för `mode`-inputen (GitHub 422 vid okända värden), lade till `tickers`-input för targeted-läge. (2) `admin.py`: ny `_WORKFLOW_INPUTS`-registry (map workflow→tillåtna inputs) + `_dispatch_workflow()` (filtrerar bort otillåtna keys → aldrig 422), nytt `POST /api/admin/workflow/trigger` (generisk, använder registry), `GET /api/admin/workflow/list`. Legacy `POST /api/admin/pipeline/trigger` kvar (bakåtkompatibelt). (3) `AdminSections.tsx` `PipelineSection` ombyggd: visar alla 13 workflows grupperade i 4 kategorier (Pipeline/ML+Backtest/Universum/Verktyg), per-workflow inputs (mode-select, tickers-text, strategy-text, dry_run-toggle), per-rad status-badge (spinner/ok-länk/feltext), länk till GitHub Actions. TypeScript: 0 errors. | `.github/workflows/pipeline.yml`, `apps/api/routers/admin.py`, `apps/web/components/admin/AdminSections.tsx` |
| 2026-06-09 | mega | **Mega-projekt fas 1–5 (ML-ranker, AI-prestanda, Liknande aktier, Insider Radar, Daglig Briefing).** Fas 1: `stock-scanner-fix/core/ml_ranker.py` (LightGBM LambdaRank, 35 features, walk-forward), `core/ml_evaluation.py`, `scripts/train_ranker.py`. Utfallsloggning: migration `024_prediction_outcomes.sql`, `backend_worker/outcome_filler.py`. Fas 2: `apps/api/routers/ml_performance.py` (admin-only, 5 endpoints), `hooks/useMlPerformance.ts`, `app/(app)/ai-prestanda/` (5-flikar-dashboard m. Recharts). Fas 3: `GET /api/stocks/{ticker}/similar` förbättrad (8 features, 1000 stocks, sektor-boost), `hooks/useStock.ts:useSimilarStocks`, ny `LiknandeTab`+`SimilarStockCard` i `StockView.tsx`. Fas 4: `SignalAnalyticsView.tsx` + `strategy_lab.py:/api/signal-analytics` — var redan komplett. Fas 5a (#11): ny `GET /api/insider-radar` (endpoint i `insider.py:radar_router`, market-wide cluster-detection + join scan_results), `app/(app)/insider-radar/InsiderRadarView.tsx`, hook `useInsiderRadar`. Fas 5b (#9): `useMacroRegime` i `useMarkets.tsx`, `app/(app)/daglig-briefing/DagligBriefingView.tsx` (composite: STARK-stocks+movers+insiders+sectors+regime). NavRail: Eye (insider-radar), Newspaper (daglig-briefing). Fas 5c (#1 analytiker-agent): `Analyskommittén` var redan komplett. TypeScript: 0 errors. | `ml_ranker.py`, `ml_evaluation.py`, `ml_performance.py`, `insider.py`, `StockView.tsx`, `NavRail.tsx`, m.fl. |
| 2026-06-09 | fix | **Portfölj + marknad: livepriser & riskmått.** (1) `price` är NULL i scan_results (pipelinen fyller inte i den) → portföljen visade 0 kr. Ny `apps/api/core/prices.py` (Yahoo v8 chart via **httpx** — yfinance finns INTE i API-bundeln). Fyller Kurs/Idag/Värde + Risk-total. `change_pct` är **bråkdel** (frontend `Intl` percent ×100). (2) Globala index: Finnhub free ger 0, yfinance-fallback död → ny `_fetch_indices_yahoo` (httpx). (3) Riskmått (Sharpe/Sortino/MaxDD/VaR/CVaR/CAGR) var tomma utan nattlig cache → ny `apps/api/core/risk_calc.py` (ren Python, 1y historik via httpx). (4) "Sälj 0k": frontend delade belopp med 1000 → visar nu faktiska kr. (5) Kalender-scope: mine=watchlist+portfölj, all=universum (konsekvent). (6) `DELETE /api/portfolio/reset` + "Töm portfölj"-knapp. | `prices.py`, `risk_calc.py`, `markets.py`, `risk.py`, `calendar.py`, `portfolio.py`, `RiskView.tsx`, `PortfoljView.tsx` |
...
| 019 | Risk analytics: portfolio_risk_cache, portfolio_factor_exposure, rebalancing_targets |
| 020 | Smart alerts: alert_rules, triggered_alerts, score_history, signal_transitions, digest_log |
| 021 | Strategy lab: strategies, strategy_runs, strategy_daily_equity, signal_persistence_cache |
| 022 | Fund holdings: fund_holdings |
| **023** | **GRANT-table-privileges (kritisk — körs manuellt)** |
| 024 | prediction_outcomes |
| 025 | pipeline_runs run_type CHECK-utökning |
| 026 | company_profiles |
| 027 | insider_trades dedup-constraint |
| **028** | **MEWS-kolumner på scan_results (mews_score, mews_flag, 6 delfaktorer)** |
| **029** | **Insider cluster signals (insider_cluster_signals-tabell) + ISIN i company_profiles** |
| **030** | **Doc intelligence: company_documents, document_chunks (pgvector), qualitative_signals** |
| **031** | **ML uncertainty + regime på scan_results (ml_uncertainty, ml_flag_uncertain, regime_at_scan)** |
| **032** | **Risk profile: user_risk_profiles (Black-Litterman #19)** |
