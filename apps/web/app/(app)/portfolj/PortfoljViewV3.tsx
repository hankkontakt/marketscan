...
import {
  formatPrice, formatPctChange, changeClass,
} from "@/lib/format";
...
import {
  ...
  badgeFor, FALLBACK_THESIS, FALLBACK_SETUP, FALLBACK_RISK,
  type BadgeStyle,
} from "@/components/screener-v3/badges";

/**
 * V3-beslutsdata på holdings (additiv berikning från current_decisions_v3 —
 * se apps/api/schemas/portfolio.py HoldingOut). Fälten saknas när innehavet
 * inte har en publicerad decision. Den delade Holding-typen i
 * types/portfolio.ts har inte fälten, så vi definierar en lokal vy-typ här
 * (delad typfil ändras inte). Saknad V3-data visas som "—", aldrig fallback
 * till legacy score_total/entry_signal.
 */
interface HoldingV3 {
  id: string;
  ticker: string;
  ...
  tradability_state?: string | null;
  is_actionable?: boolean | null;
  v3_snapshot_id?: string | null;
}

export function PortfoljViewV3() {
  ...
  const holdings = (portfolio?.holdings ?? []) as HoldingV3[];
  ...
  async function askAI() {
    ...
    const context = {
      ...
      holdings: holdings.map((h) => ({
        ...
        price: h.price,
        thesis_band: h.thesis_band, setup_state: h.setup_state,
        risk_state: h.risk_state, data_grade: h.data_grade,
      })),
    };
    try {
              ...
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr className="bg-[var(--color-bg-surface)]" style={{ borderBottom: "1px solid var(--color-border)" }}>
                    {["Aktie", "Antal", "Kurs", "Idag", "Värde", "Thesis", "Setup", "Risk", "Data"].map((h) => (
                      <th key={h} className="px-4 py-2.5 text-left text-[11px] font-medium text-[var(--color-text-muted)]">
                        {h}
                      </th>
                    ))}
                    <th className="px-4 py-2.5 text-left text-[11px] font-medium text-[var(--color-text-muted)]">
                      <span className="inline-flex items-center">
                        MasterRank
                        <InfoTooltip text="MasterRank/Thesis-poäng" />
                      </span>
                    </th>
                    <th className="px-4 py-2.5 text-left text-[11px] font-medium text-[var(--color-text-muted)]">Beslut</th>
                    <th className="px-4 py-2.5 text-left text-[11px] font-medium text-[var(--color-text-muted)]"></th>
                  </tr>
                </thead>
                <tbody>
                  {holdings.map((h) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <StateBadge value={h.thesis_band} config={THESIS_BAND_CONFIG} fallback={FALLBACK_THESIS} />
                      </td>
                      <td className="px-4 py-3">
                        <StateBadge value={h.setup_state} config={SETUP_STATE_CONFIG} fallback={FALLBACK_SETUP} />
                      </td>
                      <td className="px-4 py-3">
                        <StateBadge value={h.risk_state} config={RISK_STATE_CONFIG} fallback={FALLBACK_RISK} />
                      </td>
                      <td className="px-4 py-3">
                        <DataGradeBadge value={h.data_grade} />
                      </td>
                      <td className="px-4 py-3">
                        {h.master_rank_score != null ? (
                          <span className="font-mono font-bold text-xs text-[var(--color-text-primary)]">
                            {h.master_rank_score.toFixed(0)}
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-3">
                        {h.decision_id ? (
                          <Link
                            href={`/aktie/${h.ticker}`}
                            title={h.decision_id}
                            className="inline-block max-w-20 truncate align-middle font-mono text-[10px] text-[var(--color-text-muted)] hover:text-[var(--color-accent)] transition-colors"
                          >
                            {h.decision_id}
                          </Link>
                        ) : (
...
// ─── V3 badges ────────────────────────────────────────────────────────────────
// Text + ikon/etikett bär betydelsen, färg aldrig ensamt (samma princip som
// screener-v3/badges.ts). Saknad V3-data → "—" explicit, aldrig fallback.

function StateBadge({ value, config, fallback }: {
  value?: string | null;
  config: Record<string, BadgeStyle>;
  fallback: BadgeStyle;
}) {
  if (value == null) return <span className="text-[var(--color-text-muted)]">—</span>;
  const s = badgeFor(config, value, fallback);
  return (
    <span className={cn("inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium border", s.bg, s.text, s.border)}>
      {s.label}
    </span>
  );
}

function DataGradeBadge({ value }: { value?: string | null }) {
  if (value == null) return <span className="text-[var(--color-text-muted)]">—</span>;
  const s = DATA_GRADE_CONFIG[value] ?? { label: value, bg: "bg-zinc-500/15", text: "text-zinc-500" };
  return (
    <span className={cn("inline-flex items-center justify-center w-5 h-5 rounded text-xs font-bold", s.bg, s.text)}>
      {s.label}
    </span>
  );
}
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl overflow-hidden border border-[var(--color-border)]">
        <div className="flex gap-4 px-4 py-2.5 border-b border-[var(--color-border)] bg-[var(--color-bg-surface)]">
          {["Aktie", "Antal", "Kurs", "Idag", "Värde", "Thesis", "Setup", "Risk", "Data", "MasterRank", "Beslut", ""].map((h) => (
